...
## Structure d'un biome : un dossier par thème

Chaque mini-biome vit dans **`web/src/scene/biomes/<nom>/`** (sauf la spec E2E, qui
doit rester sous `e2e/` — voir plus bas) :

```
web/src/scene/biomes/clairiere/
  builder.ts     # construit la structure 3D (navigateur/Three.js)
  metrics.ts     # métriques photométriques SPÉCIALISÉES du thème (pur)
  manifest.ts    # identité : name, conceptImage, testUrl, criteria
  concept.png    # image de référence
e2e/clairiere.spec.ts   # spec Playwright (importe le dossier du biome)
```

> **Pourquoi la spec reste dans `e2e/`** : elle utilise `@playwright/test` et le
> `node:zlib`/`Buffer` du kit ; sous `web/src` elle casserait le typecheck navigateur
> **et** serait ramassée par Vitest (qui matche aussi `.spec.ts`). Elle importe le
> dossier du biome via `../web/src/scene/biomes/<nom>/`.

## Boîte à outils

| Fichier | Rôle | Portée |
|---------|------|--------|
| `e2e/png.ts` | Décode un PNG (zlib natif, sans dépendance) | 🔁 partagé |
| `e2e/imageDescriptor.ts` | Descripteur **générique** (luminance, saturation, vignette, symétrie, **histogramme de teintes 12 secteurs**, grille 3×3) + `similarity()` | 🔁 tout thème |
| `e2e/captureBiome.ts` | Capture un rendu carré du harnais (`toDataURL`) → PNG décodé | 🔁 partagé |
| `e2e/analyzeBiome.ts` | CLI générique : `node … <concept.png> [<render.png>]` → descripteur + score | 🔁 partagé |
| `web/src/scene/biomes/<nom>/metrics.ts` | Métriques **spécialisées** du thème (ex. cimetière : green/earth/grave) | 📦 par biome |
| `?test<Biome>=<seed>` dans `web/src/main.ts` | Harnais de scène isolé (caméra canonique, grade, capture) | motif à copier |
...
personne à hauteur d'œil, cadrage représentatif. La ranger dans le dossier du biome :
`web/src/scene/biomes/<nom>/concept.png`.

### 1. Extraire le vecteur de référence
```bash
node --experimental-strip-types e2e/analyzeBiome.ts web/src/scene/biomes/<nom>/concept.png
```
...
### 6. Boucle de mesure
Un test E2E (cf. `e2e/clairiere.spec.ts`) : `captureBiome()` → `analyze()`/`describe()`
→ `similarity()` vs le concept du manifeste, imprime le score et échoue sous un seuil.
```bash
...
1. Créer `web/src/scene/biomes/<nom>/` avec `concept.png` (skill `gpt-image-2`) et
   `manifest.ts` (copier celui de `clairiere/`, adapter name/conceptImage/testUrl).
2. `node e2e/analyzeBiome.ts web/src/scene/biomes/<nom>/concept.png` → vecteur cible.
3. `web/src/scene/biomes/<nom>/builder.ts` : builder de la structure (copier la forme de
   `clairiere/builder.ts` : Clearing/anneaux, dispose des ressources clonées).
...
5. `web/src/scene/biomes/<nom>/metrics.ts` (optionnel) si des métriques nommées
   propres au thème sont utiles ; sinon le descripteur générique suffit.
6. `e2e/<nom>.spec.ts` : `captureBiome(page, manifest.testUrl)` + `similarityGeneric()`
   vs `manifest.conceptImage`. Itérer (étapes 5–7), puis intégrer (étape 8).
